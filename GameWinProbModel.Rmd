...
savecolumns <- function(marchmadness){
  ...
  #marchmadness$matchup <- gsub("vs.", "at", marchmadness$matchup)
  return(marchmadness)
}
...
```{r}
#MarchMadness22 <- merge(AllData, MarchMadness22, by.x = "matchup", by.y = "matchup")
...
matrix_march_training <- as.matrix(march_training[,-c(160:164)])
march_model <- xgboost(data = matrix_march_training, label = as.matrix(training_labels), max.depth = 2, eta = 1, nthread = 2, nrounds = 2, model = "binary:logistic")
